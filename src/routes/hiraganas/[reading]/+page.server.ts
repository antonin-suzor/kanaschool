...
export async function load({ params, platform }) {
    ...
    const kana = await db.getKanaByReading(platform.env.D1_DB, params.reading, false);

    if (!kana) {
        throw error(404, 'Hiragana not found');
    }

    // For "ji" reading, get both the chi and shi variants
    let alternativeKana: Kana | null = null;
    if (kana.reading === 'ji') {
        const jis = await db.getHiraganas(platform.env.D1_DB);
